...
#setwd("H:/PROYECTO FINAL/BASE DATOS")
...
bd_ventas<-reshape(bd_ventas, timevar = "meses_pasados", idvar ="rut", direction = "wide")
names(bd_ventas)
...
bd_ventas1$tipo_dif_6 <- ifelse(bd_ventas1$dif_rel6< .05, "No Crece",
                                              "Crece"
)


#Graficos incorporados por mi para ver el rango de crecimiento relatico a los 6 y 12 meses
...
bd_ventas1$tipo_dif_12 <- ifelse(bd_ventas1$dif_rel12< .05, "No Crece",
                                                     "Crece"
                                                     )
...
bd_ventas_norm <- cbind(bd_ventas2[,1:3], bd_ventas_norm, bd_ventas2[8], bd_ventas2[,20:28], bd_ventas2[12])
...
bd_ventas_norm_6 <-bd_ventas_norm_6[c(1:6,8:16,7,17)]
bd_ventas_norm_12 <-bd_ventas_norm_12[c(1:6,8:16,7,17)]
...
bd_ventas_norm_12_eval$eval <- NULL
bd_ventas_norm_12_train$eval <- NULL
bd_ventas_norm_6_eval$eval <- NULL
bd_ventas_norm_6_train$eval <- NULL
...
labs(title="Distrib. de Tipos de Variación Ventas a los 6 meses", 
     x="Tipo de variación")


ggplot(data =  subset(bd_ventas1,!is.na(tipo_dif_12))) + 
  geom_bar(mapping = aes(x =tipo_dif_12, y=..prop.., group=1))+
  labs(title="Distrib. de Tipos de Variación Ventas a los  12 meses", 
       x="Tipo de variacion")
...
ggplot(data =  subset(bd_ventas2,!is.na(dif_rel12))) + 
  ...
  ggtitle("Relacion entre Tasa de crecim. a 12 meses y Ventas")
...
ggplot(data =  subset(bd_ventas2,!is.na(tipo_dif_6))) + 
  ...
  coord_cartesian(ylim = c(-1, 1))+
  theme(axis.text.x=element_text(angle=90,hjust=1))+
  ggtitle("Relacion entre Tasa de crecim. a 6 meses y Ventas")
...
ggplot(data = subset(bd_ventas2,!is.na(tipo_dif_12))) +
  ...
  ggtitle("Relación entre Tasa de crecim. 
           a 12 meses y Tipo Benef.")

ggplot(data = subset(bd_ventas2,!is.na(tipo_dif_6))) +
  geom_bin2d(mapping = aes(y = tipo, x =tipo_dif_6))+
  theme(axis.text.x=element_text(angle=20,hjust=1))+
  ggtitle("Relación entre entre Tasa de crecim.
          a 6 meses y Tipo de Benef.")
...
ggplot(data = subset(bd_ventas2,!is.na(tipo_dif_6))) +
  ...
  theme(axis.text.x=element_text(angle=90,hjust=1))+
  ggtitle("Relación entre entre Tasa de crecim. a 6 meses y Región")


ggplot(data = subset(bd_ventas2,!is.na(tipo_dif_6))) + 
  geom_bar(mapping = aes(x =as.factor(as.numeric(as.character(region)))))+ 
  facet_wrap(~ tipo_dif_6, nrow = 3, scales = "free_y")+
  ggtitle("Relación entre Tasa de crecim. a 6 meses y Región")

ggplot(data = subset(bd_ventas2,!is.na(tipo_dif_12))) + 
  geom_bar(mapping = aes(x =as.factor(as.numeric(as.character(region)))))+ 
  facet_wrap(~ tipo_dif_12, nrow = 3, scales = "free_y")+
  ggtitle("Relación entre Tasa de crecim. a 12 meses y Región")


ggplot(data = subset(bd_ventas2,!is.na(tipo_dif_6))) + 
  geom_bar(mapping = aes(x =objeto_op))+ 
  facet_wrap(~ tipo_dif_6, nrow = 3, scales = "free_y")+
  ggtitle("Relación entre Tasa de crecim. a 6 meses y Objetivo")
...
ggplot(data = subset(bd_ventas2,!is.na(tipo_dif_12))) + 
  ...
  facet_wrap(~ tipo_dif_12, nrow = 3, scales = "free_y")+
  ggtitle("Relación entre Tasa de crecim. a 12 meses y Activ. Econom.")
